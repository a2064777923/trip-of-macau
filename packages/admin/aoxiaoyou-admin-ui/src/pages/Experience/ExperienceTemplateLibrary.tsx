...
const ExperienceTemplateLibrary: React.FC = () => {
  ...
  const columns: ColumnsType<AdminExperienceTemplateItem> = [
    ...
    { title: '類型', dataIndex: 'templateType', width: 120, render: (value) => <Tag>{value}</Tag> },
    { title: '分類', dataIndex: 'category', width: 150, render: (value) => <Text ellipsis={{ tooltip: value }}>{value || '-'}</Text> },
    ...
    <div className="experience-workbench-shell">
      <Card className="experience-workbench-hero">
        ...
        <Paragraph style={{ marginBottom: 8 }}>
          模板用來沉澱可重用的展示、觸發、任務玩法與獎勵演出；從預設複製後再按故事或地點微調。
        </Paragraph>
        <Paragraph style={{ marginBottom: 0 }}>
          使用處會顯示哪些流程、章節或治理項目正在引用此模板。操作員先從安全預設開始，再按場景調整結構化欄位；進階 JSON 只作折疊補充。
        </Paragraph>
      </Card>
      ...
      <Alert
        type="info"
        showIcon
        message="模板庫不是故事內容編輯器"
        description="內容積木庫存放圖文、音頻、影片、Lottie 等可展示內容；模板庫存放「怎樣出現、怎樣觸發、觸發後做什麼」的行為規格。建立模板後，要到體驗流程工作台或故事章節覆寫中套用。"
      />
