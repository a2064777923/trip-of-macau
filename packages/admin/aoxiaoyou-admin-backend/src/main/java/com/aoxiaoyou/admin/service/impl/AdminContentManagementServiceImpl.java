...
public class AdminContentManagementServiceImpl implements AdminContentManagementService {
    ...
    @Override
    public PageResponse<AdminContentAssetResponse> pageAssets(long pageNum, long pageSize, String assetKind, String status, String keyword) {
        Page<ContentAsset> page = contentAssetMapper.selectPage(new Page<>(pageNum, pageSize),
                new LambdaQueryWrapper<ContentAsset>()
                        ...
                        .and(StringUtils.hasText(keyword), q -> q
                                .like(ContentAsset::getObjectKey, keyword)
    ...
    @Override
    public AdminContentAssetResponse uploadAsset(AdminContentAssetUploadRequest request) {
        StoredAssetMetadata storedAssetMetadata = cosAssetStorageService.storeAsset(
                request.getFile(),
        ...
        item.setBucketName(storedAssetMetadata.getBucketName());
        item.setRegion(storedAssetMetadata.getRegion());
        item.setObjectKey(storedAssetMetadata.getObjectKey());
        item.setCanonicalUrl(storedAssetMetadata.getCanonicalUrl());
        item.setMimeType(storedAssetMetadata.getMimeType());
        item.setLocaleCode(storedAssetMetadata.getLocaleCode());
        item.setFileSizeBytes(storedAssetMetadata.getFileSizeBytes());
        item.setWidthPx(storedAssetMetadata.getWidthPx());
        item.setHeightPx(storedAssetMetadata.getHeightPx());
        item.setChecksum(storedAssetMetadata.getChecksum());
        item.setEtag(storedAssetMetadata.getEtag());
