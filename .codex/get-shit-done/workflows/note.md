...
- **Global scope**: `D:/Archive/trip-of-macau/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Archive/trip-of-macau/.codex/notes/`)
...
2. Glob `D:/Archive/trip-of-macau/.codex/notes/*.md` (if directory exists) — global notes
...
Global (D:/Archive/trip-of-macau/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Archive/trip-of-macau/.codex/notes/` — works in any directory
