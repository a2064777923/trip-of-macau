# Requirements Archive: v3.1 Material Production and Mini-program Experience Acceptance

**Archived:** 2026-05-04  
**Source:** `.planning/REQUIREMENTS.md`  
**Milestone result:** Shipped with accepted WeChat UAT caveat and deferred complex gameplay engines.

## Requirement Outcomes

### Material Production and Asset Promotion

- [x] **MAT-01**: Operators can generate or import real flagship story still images from the Phase 33 manifest, including story covers, chapter hero art, pickup icons, honor/title icons, and fallback posters, while preserving prompt provenance and UTF-8 metadata.
- [x] **MAT-02**: Operators can use a material-board workflow where one generated image can contain multiple aligned assets, then crop/slice selected regions into reusable child assets with local files, COS object keys, and `content_assets` records.
- [x] **MAT-03**: Operators can synthesize narration, short sound cues, and chapter audio from approved scripts through configured providers such as CosyVoice, with language selection, voice selection, task history, preview, retry, and COS upload.
- [x] **MAT-04**: Operators can assemble simple story videos from one or more still images using pan/zoom/motion, narration/audio, subtitles or captions where needed, and export them as COS-backed video assets.
- [x] **MAT-05**: Operators can promote manifest materials from `planned` to `generated`, `uploaded`, `approved`, or `published`, with rollback to previous asset versions and clear links back to the Phase 33 story material package.
...
- [x] **QA-04**: Operators can reuse approved assets from the story material package in content blocks, rewards, pickups, POI/story overlays, and public runtime DTOs through the existing media picker components.
...
| QA-04 | Phase 37 | Complete with caveat: reuse is provided through existing media picker/library metadata and search, not a separate asset-production workbench |
...
| ACC-02 | Phase 40 | Complete with caveat: checklist exists and local DevTools/backend can be opened; actual human device pass remains pending in `40-UAT.md` |
...
- `40-UAT.md` remains pending for human WeChat DevTools/device evidence.
- Full AR/photo recognition, speech-input NPC interactions, puzzle/minigame engines, cannon-defense gameplay, route-coverage minigames, and production indoor visual positioning are future gameplay scope.
- Quick smoke skips live provider/COS mutations by default; live material proof is preserved in Phase 36 evidence.
