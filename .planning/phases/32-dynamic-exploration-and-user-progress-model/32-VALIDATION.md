...
| 32-05-02 | 05 | 3 | USER-01, USER-02, USER-03 | T32-03 | Timeline aggregation is paginated, filtered, limited to verified MySQL sources, and the workbench shows legacy `traveler_progress` as a compatibility-only snapshot distinct from dynamic weighted progress. | unit/service | `mvn -q -Dtest=AdminUserTimelineServiceTest test -f packages/admin/aoxiaoyou-admin-backend/pom.xml` | ❌ W0 | ⬜ pending |
...
| 32-04-02 | 04 | 4 | USER-01, USER-02, USER-03, USER-04, LINK-03 | T32-05 | Admin workbench uses authenticated admin APIs, guarded repair actions, Traditional Chinese UI copy, and a `legacyProgressSnapshot` compatibility card that contrasts `traveler_progress` with dynamic weighted progress. | build/smoke | `npm run build --prefix packages/admin/aoxiaoyou-admin-ui; powershell -ExecutionPolicy Bypass -File scripts/local/smoke-phase-32-user-progress.ps1` | ❌ W0 | ⬜ pending |
...
- [ ] `packages/admin/aoxiaoyou-admin-backend/src/test/java/com/aoxiaoyou/admin/AdminUserTimelineServiceTest.java` — timeline fan-in coverage for check-ins, triggers, exploration events, reward history, sessions, legacy `traveler_progress` compatibility snapshots, and trace-unavailable fallback.
...
| Legacy progress compatibility card | USER-01, USER-02, LINK-03 | Requires visual confirmation that operators can distinguish legacy snapshots from current dynamic progress. | Open `/admin/#/users/progress/{seededUserId}`, verify `進度總覽` shows `舊進度快照（兼容）`, source `traveler_progress`, and explanatory copy that the value is not used for the current dynamic weighted percentage. |
