...
| 36-05-02 | 05 | 3 | MAT-04 | T36-21/T36-22/T36-24 | Video build runs only after ffmpeg subtitle support and published narration checks pass | CLI smoke | `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/local/material-production/phase36-build-video.ps1 -Config docs/content-packages/east-west-war-and-coexistence/production-runs/phase36-video-jobs.json -ValidateOnly` | ✅ | ❌ red: `FFMPEG_SUBTITLES_UNAVAILABLE` |
| 36-04-01 | 04 | 4 | MAT-01..MAT-05 | T36-16..T36-20 | Live smoke proves COS-backed asset binding, status promotion, rollback, subtitle gating, and no unsafe inline Chinese writes | smoke | `powershell -ExecutionPolicy Bypass -File scripts/local/smoke-phase-36-material-production.ps1` | ✅ | ❌ red: missing `PHASE36_ADMIN_BEARER_TOKEN`; provider/COS/ffmpeg gates blocked |
...
**Approval:** blocked on live admin auth, provider/COS readiness, and ffmpeg subtitle support
