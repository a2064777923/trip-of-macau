...
The planner should split Phase 36 into four execution plans: `P1` backend schema/orchestration/versioning, `P2` local production/import tooling, `P3` package-scoped admin UI actions, and `P4` smoke/dependency verification with the MP4 slice gated on `ffmpeg` availability. This split matches the actual dependency order in the codebase and prevents MAT-04 from blocking MAT-01/02/03/05. [VERIFIED: .planning/ROADMAP.md; workstation env audit 2026-04-30]
...
What's unclear: Whether the planner wants one unified history surface for both backend TTS and local image/video generation, or only reliable rollback/queryability. [VERIFIED: .planning/phases/36-material-production-pipeline-and-asset-promotion/36-CONTEXT.md]
Recommendation: Prefer a dedicated material version journal as the non-negotiable core, then add optional foreign keys back to AI job/candidate rows when a version was produced through the backend AI path. [VERIFIED: .planning/phases/36-material-production-pipeline-and-asset-promotion/36-CONTEXT.md]
...
What's unclear: Whether a short nonverbal “reward unlock” cue will be acceptable as a stylized spoken audio output, or whether the asset should be manually produced/imported instead. [VERIFIED: docs/content-packages/east-west-war-and-coexistence/audio-scripts.md]
Recommendation: Do not let this ambiguity block narration or video work; plan `sfx_reward_unlock` as a small branch with an explicit fallback to manual import if CosyVoice output is not acceptable. [VERIFIED: .planning/REQUIREMENTS.md]
